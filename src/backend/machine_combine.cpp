...
void combineInstruction(MachineFunction& function, const DefinitionInfo& definitions,
                        const MachineCombineOptions& options, MachinePassResult& result,
                        bool& changed, MInstruction& instruction,
                        std::vector<MInstruction>& combined) {
    ...
    if (options.reducePowerOfTwoMultiply && instruction.opcode == MOpcode::MUL &&
        ...
        for (const std::size_t constantIndex : {std::size_t{1}, std::size_t{0}}) {
            ...
            for (unsigned shift = 1; shift <= 30; ++shift) {
                ...
                // The inserted arithmetic is visited right after the mutated
                // shift, exactly like the old index-based loop did.
                combineInstruction(function, definitions, options, result, changed,
                                   arithmetic, combined);
                changed = true;
                expanded = true;
        ...
        for (const std::size_t constantIndex : {std::size_t{1}, std::size_t{0}}) {
            ...
            const std::size_t replacementCount = sequence.size();
            for (auto& replacement : sequence)
                combined.push_back(std::move(replacement));
            changed = true;
            expanded = true;
            result.instructionsRewritten += replacementCount;
            break;
        }
        if (expanded) return;
    }
...
bool targetCombine(MachineFunction& function, const MachineCombineOptions& options,
    ...
    for (auto& block : function.blocks) {
        // Expansions are appended to a fresh instruction list instead of
        // erasing/inserting in the middle of the block, which made every
        // expansion cost O(block size).
        std::vector<MInstruction> combined;
        combined.reserve(block.instructions.size());
        for (auto& instruction : block.instructions)
            combineInstruction(function, definitions, options, result, changed,
                               instruction, combined);
        block.instructions = std::move(combined);
    }
    return changed;
}

// Propagating a copy only ever removes a definition (its destination) and
// rewrites uses of that destination, so a copy that is ineligible when first
// visited stays ineligible for the rest of the pass. A single forward scan
// therefore propagates exactly the copies the old rescan-from-scratch loop
// would pick, in the same order, while needing only one definition count pass
// and one use rewrite per propagated copy.
bool propagateCopies(MachineFunction& function, MachinePassResult& result) {
    std::vector<unsigned> counts(function.vregCount, 0);
    std::vector<const MInstruction*> definers(function.vregCount, nullptr);
    for (const auto& block : function.blocks)
        for (const auto& instruction : block.instructions)
            for (const auto& operand : instruction.defs)
                if (const auto* reg = std::get_if<VirtualReg>(&operand)) {
                    ++counts[reg->id];
                    definers[reg->id] = &instruction;
                }
    bool changed = false;
    std::vector<std::vector<std::size_t>> removed(function.blocks.size());
    for (std::size_t blockIndex = 0; blockIndex < function.blocks.size(); ++blockIndex) {
        ...
        for (std::size_t index = 0; index < block.instructions.size(); ++index) {
            ...
            if (!destination || !source || destination->id == source->id ||
                counts[destination->id] != 1 || counts[source->id] != 1)
                continue;
            // Source has exactly one definition, so only its defining
            // instruction can both define source and use destination.
            const auto& definerUses = definers[source->id]->uses;
            const bool createsSelfDependence = std::any_of(
                definerUses.begin(), definerUses.end(), [&](const MOperand& operand) {
                    const auto* reg = std::get_if<VirtualReg>(&operand);
            ...
            removed[blockIndex].push_back(index);
            result.changed = true;
    ...
    for (std::size_t blockIndex = 0; blockIndex < function.blocks.size(); ++blockIndex) {
        const auto& indices = removed[blockIndex];
        if (indices.empty()) continue;
        auto& instructions = function.blocks[blockIndex].instructions;
        std::vector<MInstruction> kept;
        kept.reserve(instructions.size() - indices.size());
        std::size_t next = 0;
        for (std::size_t index = 0; index < instructions.size(); ++index) {
            if (next < indices.size() && indices[next] == index) {
                ++next;
                continue;
            }
...
MachinePassResult runPreRAMachineCombine(
    ...
    do {
        ...
        if (options.propagateVirtualCopies && propagateCopies(function, result))
            localChanged = true;
